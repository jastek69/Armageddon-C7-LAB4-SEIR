...
Required env vars:
  ...
  DOMAIN_NAME         Domain name served by CloudFront (e.g. kingdomcome.com)
...
Recommended env vars:
  ...
  ORIGIN_REGION       Region of origin resources (default: us-west-2)
...
if [[ "$REQUIRE_WAF_ASSOCIATION" == "true" ]]; then
  # Use CloudFront config directly (more reliable than wafv2 get-web-acl-for-resource for CloudFront)
  ...
  if [[ -n "$waf_assoc_arn" && "$waf_assoc_arn" != "None" ]]; then
    ...
    managed_count="$(aws wafv2 get-web-acl --id "$(echo "$waf_assoc_arn" | awk -F/ '{print $NF}')" \
      --name "$(echo "$waf_assoc_arn" | awk -F/ '{print $(NF-1)}')" \
...
check_alias_record() {
  ...
  # Route53 returns targets with trailing dot; normalize both sides for comparison
  normalized_target="$(echo "$target" | sed 's/\\.$//' | awk '{$1=$1;print}' | tr -d '\r')"
  normalized_found="$(echo "$found_target" | tr '\t' '\n' | sed 's/\\.$//' | awk '{$1=$1;print}' | tr -d '\r')"
  if echo "$normalized_found" | grep -qi "$normalized_target"; then
...
if [[ -n "$LOG_BUCKET" ]]; then
  ...
  normalized_expected="$(echo "$expected_cf_bucket" | tr -d '\r' | tr -d ' ' | sed 's/\\.$//')"
  normalized_logging_bucket="$(echo "$logging_bucket" | tr -d '\r' | tr -d ' ' | sed 's/\\.$//')"
  if [[ "$logging_enabled" == "true" ]]; then
    if [[ "${normalized_logging_bucket,,}" == "${normalized_expected,,}" ]]; then
...
if [[ -n "$ORIGIN_SG_ID" ]]; then
  ...
  for p in "${ports[@]}"; do
    ...
    v6="$(aws ec2 describe-security-groups --group-ids "$ORIGIN_SG_ID" --region "$ORIGIN_REGION" \
      ...
      --output text 2>/dev/null || echo "")"
    plids="$(aws ec2 describe-security-groups --group-ids "$ORIGIN_SG_ID" --region "$ORIGIN_REGION" \
      --query "SecurityGroups[0].IpPermissions[?FromPort<=\`${p}\` && ToPort>=\`${p}\`].PrefixListIds[].PrefixListId" \
      --output text 2>/dev/null || echo "")"
    ...
    if [[ -n "$plids" ]]; then
      add_detail "PASS: Origin SG $ORIGIN_SG_ID allows prefix list(s) on port $p ($plids)."
    elif [[ -z "$src_sgs" && -z "$v4" && -z "$v6" ]]; then

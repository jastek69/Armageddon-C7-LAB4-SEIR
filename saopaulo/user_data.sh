...
retry_dnf() {
  ...
  for i in $(seq 1 "$tries"); do
    ...
    echo "dnf failed (attempt $i/$tries). Retrying in ${delay}s..."
    sleep "$delay"
  done
  return 1
}

retry_dnf "dnf -y update"

# Core runtime + debug tooling (single install)
retry_dnf "dnf -y install \
  python3 python3-pip \
...
[Unit]
Description=São Paulo EC2 Compute Spoke App
...
<body>
  <div class="bgimg w3-display-container w3-animate-opacity w3-text-white">
    ...
    <div class="w3-display-bottomleft w3-padding-small transparent-background outlined-text">
      <h1>"São Paulo Compute Spoke Region - Database Consumer by John Sweeney"</h1>
      <h3>Team Group Co-Leader is T.I.Q.S. I am Blackneto of the Brotherhood of Evil jerMutants</h3>
      <h3><b>Region:</b> sa-east-1 (São Paulo) - Compute Spoke Region</h3>
      <p><b>Database Access:</b> via Tokyo Transit Gateway</p>

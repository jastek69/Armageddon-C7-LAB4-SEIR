...
Repository Layout
LAB4/
├── Tokyo/             # Primary AWS region (data authority + TGW hub)
│   ├── main.tf
│   ├── tgw-route-tables.tf
│   ├── bedrock-autoreport.tf
│   ├── outputs.tf
│   └── backend.tf
├── saopaulo/          # Secondary AWS region (stateless compute + TGW spoke)
│   ├── main.tf
│   ├── data.tf        # Reads Tokyo remote state
│   ├── outputs.tf
│   └── backend.tf
├── global/            # CloudFront, WAF, Route53, global edge controls
│   ├── cloudfront.tf
│   ├── waf.tf
│   ├── route53.tf
│   └── backend.tf
...
│   ├── outputs.tf
│   └── 2-backend.tf
├── modules/           # Reusable Terraform modules (translation, monitoring, logging, etc.)
├── lambda/            # Lambda source (IR reporting, alarm hooks, rotation helpers)
├── python/            # Validation, CLI, and automation scripts
└── scripts/           # Apply/destroy and operational helper scripts

🚆 Naming Conventions (Important)

To make the architecture feel local and intentional:
Tokyo (train stations / hub naming)
    shinjuku-*
    shibuya-*
    ueno-*
    akihabara-*
...
New York GCP deployment
  nihonmachi-*

Shared project/service prefixes
  taaops-*
  galactus-*
...
Tokyo (expanded hub)
  Keep Lab 2 app/data foundation in ap-northeast-1
  Add TGW hub, VPN attachments, and route table segmentation
  Add IR pipeline (CloudWatch/SNS/Lambda/Bedrock/S3)
  Add translation handoff to S3-triggered translation Lambda
  Keep PHI data authority and RDS in Tokyo

São Paulo (spoke region)
  Deploy Lab 2-style stateless compute stack without regional RDS
  Connect as TGW spoke to Tokyo
  Route Tokyo/GCP CIDRs through TGW corridor
  Read infra dependencies from Tokyo remote state

Global stack (edge/control plane)
  CloudFront + WAF + Route53 global entrypoint
  Origin protection and centralized edge policy controls

New York GCP stack (cross-cloud extension)
  Private GCP app stack with internal HTTPS ILB + MIG
  HA VPN + BGP to AWS TGW (Tokyo)
  No persistent PHI storage in GCP
...
## Appendix: IR + Translation Workflow (Current Deployment)

Figure A1. Current deployed control/data flow in this repository. SNS is used for alarm fan-out and IR report-ready notifications. Translation orchestration is S3 event-driven to Lambda.
...
Figure A2. Sequence view for operational runbook alignment.
...
Assumptions:
- No Step Functions state machine is currently deployed for the IR or translation path.
- Translation starts from S3 ObjectCreated events on the translation input bucket.
- CloudWatch Logs feed IR evidence via Logs Insights queries executed by the IR Lambda.

### Terraform Graph

Terraform dependency graph for the LAB4 network and cross-cloud topology:

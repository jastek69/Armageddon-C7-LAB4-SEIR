...
data "terraform_remote_state" "gcp" {
  count   = local.gcp_state_enabled ? 1 : 0
  backend = "s3"
  config = {
    bucket = var.gcp_state_bucket
    key    = var.gcp_state_key
    region = var.gcp_state_region
  }
}
...
locals {
  ...
  effective_tgw_id = var.tgw_id != "" ? var.tgw_id : aws_ec2_transit_gateway.shinjuku_tgw01.id
}
...
############################################################
# Transit Gateway New York (Iowa) - AWS - GCP
############################################################

# Customer Gateway for GCP
# https://registry.terraform.io/providers/hashicorp/aws/3.74.1/docs/resources/customer_gateway
...
resource "aws_vpn_connection" "tgw_vpn_1" {
  ...
  customer_gateway_id = aws_customer_gateway.gcp_cgw_1[0].id
  type                = "ipsec.1"
...
resource "aws_vpn_connection" "tgw_vpn_2" {
  count               = local.gcp_vpn_ready ? 1 : 0
  transit_gateway_id  = local.effective_tgw_id
  customer_gateway_id = aws_customer_gateway.gcp_cgw_2[0].id
  type                = "ipsec.1"
  static_routes_only  = false
